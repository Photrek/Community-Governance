# TODO: In deployed container, run a cronjob with python manage.py cleanup_temp_dir

import os

from django.conf import settings
from django.http import Http404, HttpResponseNotFound, FileResponse
    ...
    def handle_upload_button(self, request):
        ...
        try:
            ...
            try:
                ...
                error_message = "The provided file is not in a valid format. Please provide a zip file."
                raise e
            ...
            try:
                ...
                error_message = f"The provided zip file could not be uploaded to the server: {repr_exception(e)}"
                raise e
            ...
            try:
                ...
                error_message = "The provided zip file could not be converted to a SQLite database. Please ensure it is a valid export from the Swae portal."
                raise e

            # Get available missions from the SQLite file to display them in the next form
            try:
                missions = wf.get_mission_information()
                assert len(missions) > 0
                filter_form = FilterForm(missions=missions)
            except Exception as e:
                error_message = f"The provided zip file seems to contain not a single mission: {repr_exception(e)}"
                raise e
    ...
    def handle_filter_button(self, request):
        ...
        try:
            ...
            try:
                ...
                error_message = f"The submitted data contained an invalid item: {repr_exception(e)}"
                raise e

            # Fetch selected missions
            try:
                selected_mission_ids = [name for name, val in data.items() if val and name != 'filter_button']
                assert len(selected_mission_ids) > 0
            except Exception as e:
                error_message = "The current selection contains zero missions. Please select at least one mission."
                raise e
    ...
    def handle_calculate_scores_button(self, request):
        ...
        try:
            ...
            try:
                ...
                variables = {name: val for name, val in data.items() if name != "calculate_scores_button"}
            except Exception as e:
                error_message = f"The submitted data contained an invalid item: {repr_exception(e)}"
                raise e

            # Calculate contribution scores and reward distribution
            try:
                wf.calculate_scores(variables)
            except Exception as e:
                error_message = f"Contribution score calculation failed: {repr_exception(e)}"
                raise e

            # Fetch active users to display them as part of the next form
            try:
                users = wf.get_user_information()
                calculate_rewards_form = CalculateRewardsForm(users=users)
            except Exception as e:
                error_message = f"Could not get information about users that were active in the selected missions: {repr_exception(e)}"
                raise e
    ...
    def handle_calculate_rewards_button(self, request):
        ...
        try:
            ...
            try:
                ...
                filtered_user_ids = [name for name, val in data.items()
                                    if val is True and name in calculate_rewards_form.field_groups[1]['fields']]
            ...
            try:
                ...
                error_message = f"Convertion of SQLite file to Excel file failed: {repr_exception(e)}"
                raise e
            ...
            try:
                ...
                error_message = f"Creation of visualizations failed: {repr_exception(e)}"
                raise e
            ...
            try:
                ...
                error_message = f"Creation of zip file with results failed: {repr_exception(e)}"
                raise e
...
class ZipFileUploadForm(forms.Form):
    ...
    upload_button = forms.CharField(
        label="",
        widget=forms.TextInput(attrs={'type': 'submit', 'class': 'btn', 'id': 'zip-file-upload-button', 'value': 'Upload'}),
        required=False,
    )
...
class FilterForm(forms.Form):
    def __init__(self, *args, missions=None, **kwargs):
        ...
        for row in missions:
            m_id = str(row[0])
            m_title = str(row[1])
            self.fields[m_id] = forms.BooleanField(initial=False, required=False, label=m_title)

        self.fields['filter_button'] = forms.CharField(
            widget=forms.TextInput(attrs={'type': 'submit', 'name': 'filter-button', 'class': 'btn', 'value': 'Apply'}), label="", required=False)
...
class CalculateScoresForm(forms.Form):
    ...
    fraction_of_contribution_scores_for_highly_rated_proposals = forms.FloatField(initial=0.0)

    calculate_scores_button = forms.CharField(widget=forms.TextInput(attrs={'type': 'submit', 'class': 'btn', 'value': 'Calculate'}), label="", required=False)
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.field_groups = [
            ...
            {
                'text': 'Influence of proposal ratings: What fraction of the total score shall come from ratings?',
                'fields': [
                    'fraction_of_contribution_scores_for_highly_rated_proposals',
                ]
            },
        ]
...
class CalculateRewardsForm(forms.Form):
    ...
    function_voting_weight = forms.CharField(label="Voting power formula", initial="x**2")
    ...
    calculate_rewards_button = forms.CharField(widget=forms.TextInput(attrs={'type': 'submit', 'class': 'btn', 'value': 'Calculate'}), label="", required=False)
    ...
    def __init__(self, *args, users=None, **kwargs):
        ...
        for row in users:
            user_id = str(row[0])
            self.fields[user_id] = forms.BooleanField(initial=False, required=False, label=user_id)
